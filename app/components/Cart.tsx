...
import type {
  ...
  CartCost,
  CartLine as CartLineType,
  CartLineUpdateInput,
} from 'storefrontapi.generated';
import {Link} from 'react-router';
import {useAside} from './Aside';

type Layouts = 'page' | 'aside';
...
export function CartDetails({
  ...
  const container = {
    aside: 'grid grid-cols-1 h-[calc(100vh-5rem)] grid-rows-[1fr_auto]',
...
function CartSummary({
  ...
  const summary = {
    aside: 'grid gap-4 p-6 border-t border-neutral-200 bg-neutral-50',
...
function ItemRemoveButton({lineId}: {lineId: CartLineType['id']}) {
  return (
    <CartForm
      route="/cart"
...
export function CartEmpty({
  ...
  const container = {
    aside: 'flex flex-col items-center justify-center gap-6 px-6 py-12 h-[calc(100vh-5rem)]',
    page: hidden
      ? ''
      : 'grid pb-12 w-full md:items-start gap-4 md:gap-8 lg:gap-12',
  };
